...
export const SimpleBarChart = (props) => {
  ...
  const drawChart = () => {
    ...
    const data = dataFromProps.map(o => { return { ...o, y: Object.values(o).reduce((p, c) => p + (c > 0 ? c : 0), 0) } })
...
export const SimpleStackedBarChart = (props) => {
  ...
  const drawChart = () => {
    const stackedData = d3.stack().keys(Object.keys(dataFromProps[0]).filter(f => !['label', 'type'].includes(f)))(dataFromProps)
    ...
    const selection = svg.selectAll('rect').data(stackedData)
